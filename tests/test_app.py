...
from universal_mcp_google_ads.app import GoogleAdsApp

@pytest.fixture
def app_instance():
    """Provides a GoogleAdsApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return GoogleAdsApp(integration=mock_integration)

def test_universal_mcp_google_ads_app_initialization(app_instance):
    """
    Test that the GoogleAdsApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "google-ads", "GoogleAdsApp instance has unexpected name."


def test_universal_mcp_google_ads_tool_docstrings_format(app_instance):
    """
    Test that each tool method in GoogleAdsApp has a well-formatted docstring,
...
def test_universal_mcp_google_ads_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in GoogleAdsApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
